...
class GUI(tk.Tk):
    def __init__(self):
        ...
        names = ['Messenger', 'Help']
        self.nb = self.create_notebook(names)
        self.menu = self.create_menus()

        mess_tab = self.nb.tabs['Messenger']

        mess_lbl_frm = ttk.LabelFrame(mess_tab, text='yo')
        mess_lbl_frm.grid(row=0, column=0, sticky=tk.NSEW)

        mess_lbl = ttk.Label(mess_lbl_frm, text='yo')
    ...
    def create_notebook(self, names):
        ...
        help_tab = nb.tabs['Help']
        help_lblfrm = ttk.Labelframe(help_tab, text="Help")
        help_lblfrm.grid(row=0, column=0)
        help_lbl = ttk.Label(
            help_lblfrm, text='nothing here yet')
